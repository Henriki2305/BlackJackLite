...
var bigNumberHandScene = preload("res://Hands/BigNumberHand/BigNumberHand.tscn")
var exampleHandScene = preload("res://Hands/ExampleHand/ExampleHand.tscn")
var hands : Array = []
var movedHand
...
func _process(_delta: float) -> void:
	if movedHand:
		...
		movedHand.global_position = lerp(movedHand.global_position,mPos,10*_delta )
		for i in len(hands):
	...
	if playPhase:
		if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
			if event.pressed:
				...
				if movedHand:
					var tween = get_tree().create_tween()
					tween.tween_property(movedHand,"position", Vector2(movedHand._getPosition()*200,0),0.1)
					var h = movedHand
					movedHand = null
